#./pyspark --executor-cores 1 --total-executor-cores 1 --conf spark.default.parallelism=1 svm_spark.py
...
intmfile = 'lmoutput_bad_intm.csv'
opfile = 'lmoutput_bad_transposed.txt'
try:
    os.remove(predict)
except OSError:
    pass
...
totalgood_data = data.map(lambda s: s.split(',') and s[14] == 'bad').keyBy(lambda x: x[0]).mapValues(lambda x:(x[:])) 
